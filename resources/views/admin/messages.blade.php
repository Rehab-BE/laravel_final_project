        ...
        <table class="table table-hover table-borderless display" id="_table">
          ...
          @foreach($contacts as $contact)
            <tr>
              ...
              <td><a href="{{route('messages.show', $contact->id)}}" class="text-decoration-none text-dark">{{Str::limit(($contact->message),50)}}</a></td>
              <td>{{ $contact->name }}</td>
              <td class="text-center">
              <form action="{{ route('messages.destroy', $contact->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete?')">
                  @csrf
                  @method('DELETE')
